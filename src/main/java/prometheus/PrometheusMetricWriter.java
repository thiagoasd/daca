...
class PrometheusMetricWriter implements MetricWriter {
	...
	private Counter counter(String name) {
		...
		return this.counters.computeIfAbsent(key, k -> Counter.build().name(k).help(k).register(this.registry));
	}

	private Gauge gauge(String name) {
		String key = sanitizeName(name);
		return this.gauges.computeIfAbsent(key, k -> Gauge.build().name(k).help(k).register(registry));
	}
